...
def is_car_inside(plate_number):
    """Check if the car is already inside the parking lot."""
    try:
        ...
        print(f"[ERROR] MySQL validation failed: {e}")
        return False
                        ...
                        if (prefix.isalpha() and prefix.isupper() and
                            ...
                            if len(plate_buffer) >= 3:
                                ...
                                if is_car_inside(most_common):
                                    print(f"[DENIED] Car {most_common} is already inside the parking lot.")
                                    if arduino:
                                ...
                                if (most_common != last_saved_plate or
                                    ...
                                    try:
                                        ...
                                        cursor = conn.cursor()
                                        cursor.execute("""
                                            INSERT INTO parking_logs (plate_number, entry_timestamp, payment_status)
                                            VALUES (%s, NOW(), 'pending')
                                        """, (most_common,))
                                        conn.commit()
                                        print(f"[MYSQL] {most_common} registered in database.")
                                        cursor.close()
                                        conn.close()
                                    except Exception as e:
